...
$subKeys = Get-ChildItem -Path $basePath -Recurse -ErrorAction SilentlyContinue | Where-Object { $_.PSIsContainer }

function Remove-RegistryProperties {
    param (
        [string]$path
    )
    $propertiesToRemove = @("MinimumStackCommitInBytes", "Debugger")
    
    foreach ($property in $propertiesToRemove) {
        try {
            $parameters = Get-ItemProperty -Path $path -ErrorAction SilentlyContinue
            if ($parameters.PSObject.Properties.Name -contains $property) {
                Remove-ItemProperty -Path $path -Name $property -Force -Confirm:$false -ErrorAction SilentlyContinue
            }
        } catch {}
    }
}

Remove-RegistryProperties -path $basePath

foreach ($subKey in $subKeys) {
    Remove-RegistryProperties -path $subKey.PSPath
}
